...
class DStabilityModel(BaseModel):
    ...
    def add_point_to_list(
        self, new_point: ShapelyPoint, points: list[ShapelyPoint], tol: float = 1e-4
    ):
        ...
        for i in range(len(points) - 1):
            ...
            if new_point.equals_exact(points[i], tolerance=tol) or new_point.equals_exact(
                points[i + 1], tolerance=tol
            ):
                break
    ...
    def connect_layers(
        ...
        intersect_points_1 = [
            p
            for p in points_1
            if linestring2.distance(p) < tolerance and p not in points_2
        ]
        intersect_points_2 = [
            p
            for p in points_2
            if linestring1.distance(p) < tolerance and p not in points_1
        ]
        ...
        for points, intersect_points in [
            (points_1, intersect_points_2),
            (points_2, intersect_points_1),
        ]:
